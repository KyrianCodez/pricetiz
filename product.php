...
<?php if(!$_POST): ?>
  ...
  <script type="text/javascript">
    ...
    var productColumns = ["id", "subType", "name", "quantity", "buy_price", "sale_price", "media_id", "date", "description", "singleUnit", "itemLink", "city", "email", "phone", "zipcode", "freeShipping", "company", "website", "purchaseType", "categorie", "image"];

    var tableColumns = ["#", "Photo", "ProductType", "Product Title", "Type", "SubType", "Instock", "Price", "Product Added", "ItemLink", "Company", "Website", "City", "ZipCode", "Phone", "Actions"];
      ...
      if(productTableBody && Array.isArray(products)){
        ...
        products.forEach((p, index, arr) => {
          ...
          tableColumns.forEach((tCol, i) => {
            ...
            console.log()
            if(productCol){
               if(productCol === "itemLink" && productCol === "website"){
                  row += `<td><a target = '_blank' href="${p[productColumns.findIndex((c) => c === productCol)]}">Link</a></td>`;
               }else if(productCol === "image"){
                  let image_url = p[productColumns.findIndex((c) => c === "image")];

                  if(image_url){
                    row += `<td><img class="img-avatar img-circle" src="uploads/products/${image_url}" alt=""></td>`;
                  }else{
                    row += `<td><img class="img-avatar img-circle" src="uploads/products/no_image.jpg" alt=""></td>`;
                  }
               }else{
              ...
              products = products.filter((p) => {
                ...
                try{
                  ...
                  if(pCol){
                    ...
                    let matches = (p[productColumns.findIndex((c) => c === pCol)] || "").toString().match(regex);
